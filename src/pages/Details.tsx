...
function Details({ id, title, type, genre, minPrice, status, description, tags, rating, nbRating, avis, hoteInfo, images, menu, formules, jourDebut, jourFin, heureDebut, heureFin }: DetailsProps) {
        ...
        <div className="flex flex-col h-screen">
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto scrollbar-hide mb-[50px]">
                    <div className={`flex flex-col items-center justify-center px-[12px] pb-24 mt-[8%] sm:mt-[21%] md:mt-[8%] lg:mt-[8%] xl:mt-[8%] ${isSidebarCollapsed ? 'sm:w-full md:w-full lg:w-[87%] xl:w-[87%]' : 'sm:w-full md:w-full lg:w-[82%] xl:w-[82%]'} ml-[68px] sm:ml-0 md:ml-0 lg:ml-[68px] xl:ml-[68px] ${isSidebarCollapsed ? 'sm:w-full md:w-full lg:w-[87%] xl:w-[87%]' : 'sm:w-full md:w-full lg:w-[82%] xl:w-[82%]'}`}>
                        {/* Breadcrumb */}
                        <div className="flex flex-row gap-2 items-center justify-start w-full mb-[24px]">
                            <span className="text-[16px] sm:text-[14px] md:text-[16px] lg:text-[16px] xl:text-[16px] font-bricolagegrotesque">
                                <Link to="/hotels" className="text-sky-500 font-bricolagegrotesque">
                                    Accueil
                                </Link>
                            </span>
                            <span className="text-gray-500 font-bricolagegrotesque">/</span>
                            <span className="text-[16px] sm:text-[14px] md:text-[16px] lg:text-[16px] xl:text-[16px] text-gray-500 font-bricolagegrotesque">{title}</span>
                        </div>
                        ...
                        <div className="text-left items-start justify-center w-full mb-[24px]">
                            ...
                            {/*Health*/}
                            {type === 'Health' && (
                                <div className="flex gap-[24px] items-center mb-[11px]">
                        ...
                        {/*Rating and nbRating */}
                        <div className={`flex flex-row gap-[12px] items-center ${type === 'Health' ? 'justify-between' : 'justify-start'} w-full mb-[40px]`}>
                            ...
                            {type === 'Health' && (
                                <div className="flex gap-[6px] items-center">
                                    <button
                                        className="flex gap-[4px] items-center justify-center px-[15px] py-[6px] rounded-[12px] bg-emerald-500 transition-colors"
                                    >
                        ...
                        <div className="flex flex-col items-start justify-center w-full mb-[40px]">
                            <div className="flex flex-col gap-[16px] items-start justify-center w-full mb-[40px]">
                                <p className="text-2xl font-bold font-bricolagegrotesque">{apropos}</p>
                                <p className="text-normal text-slate-700 whitespace-pre-line font-bricolagegrotesque">{description}</p>
                            </div>
                            ...
                            <div className="flex flex-col gap-[16px] items-start justify-center w-full mb-[40px]">
                                <p className="text-2xl font-bold font-bricolagegrotesque">{ceque}</p>
                                <div className="flex flex-wrap gap-[16px] w-full">
                                    {/* Show all tags on desktop, limited on mobile */}
                                    {tags.map((tag, index) => {
                                        // On mobile (sm and below), hide tags after index 3 if not showing all
                                        const isVisible = index < 4 || showAllTags;
                                        return (
                                            <div key={tag.text} className={`${isVisible ? 'block' : 'hidden'} md:block`}>
                                                <Tags text={tag.text} />
                                            </div>
                                        );
                                    })}
                            ...
                            <div className="flex flex-col gap-[16px] items-start justify-center w-full mb-[40px]">
                                <p className='text-2xl font-bold font-bricolagegrotesque'>Découvrez {avis.length} avis fascinants</p>
                                <div className="grid grid-cols-3 gap-[80px] sm:gap-[52px] md:gap-[80px] lg:gap-[80px] xl:gap-[80px]  sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 items-start justify-center w-full">
                                    {/* Show 4 on mobile, 8 on desktop initially */}
                                    {avis.slice(0, showAllAvis ? avis.length : (isMobile ? 4 : 8)).map((avis) => (
                                    ...
                                    {/* Desktop: show button if more than 8 comments */}
                                    {!isMobile && avis.length > 8 && (
                                        <div className="md:col-span-2 lg:col-span-1 flex flex-col gap-3 items-center justify-center w-full h-full">
                                            <button
                                                onClick={() => setShowAllAvis(!showAllAvis)}
                                                className="w-[90%] h-[48px] flex flex-col items-center justify-center bg-slate-200 rounded-[12px] group"
                                            >
                            ...
                            <div className='flex flex-col gap-[16px] items-start justify-center w-full mb-[40px]'>
                                <p className='text-2xl font-bold font-bricolagegrotesque'>Où vous serez</p>
                                {/* map des images */}
                            ...
                            <div className='flex flex-col gap-[40px] items-start justify-center w-full'>
                                ...
                                {/* Health details: Show Health recommendations as hover mode cards */}
                                {type === 'Health' && healthCards.length > 0 && (
                                    <Recommendation
                                        type='Urgences'
                                        cards={healthCards}
                                    />
                        ...
                        {/* Mobile Footer */}
                        <div className={`fixed bottom-0 left-4 right-4 mb-4 rounded-[25px] p-[1.5px] flex flex-col gap-[8px] items-center justify-center z-20 sm:flex md:hidden`} style={{ background: 'radial-gradient(141.56% 141.56% at 50% -7.74%, var(--colors-teal-400, #2DD4BF) 0%, var(--colors-sky-500, #0EA5E9) 50.96%, var(--colors-fuchsia-500, #D946EF) 100%)' }}>
                            <div className='w-full bg-white rounded-[25px] p-[12px] flex flex-col gap-[8px] items-center justify-center'>
                                <button className='w-full rounded-2xl px-[24px] py-[14px] flex gap-[12px] items-center justify-center shadow-sm whitespace-nowrap' style={{ background: 'radial-gradient(262.5% 262.5% at 50% -97.5%, var(--colors-teal-400, #2DD4BF) 0%, var(--colors-sky-500, #0EA5E9) 55%, var(--colors-fuchsia-500, #D946EF) 100%)', boxShadow: '0px 1px 2px -1px rgba(0, 0, 0, 0.1), 0px 1px 3px 0px rgba(0, 0, 0, 0.1)' }}>
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
                                        ...
                                        <path d="M12 21C15.866 21 19 19.2091 19 17C19 14.7909 15.866 13 12 13C8.13401 13 5 14.7909 5 17C5 19.2091 8.13401 21 12 21Z" fill="white" />
                                    </svg>

                                    <p className='text-lg font-semibold text-white font-bricolagegrotesque leading-[28px] whitespace-nowrap'>{contactButtonText}</p>
                                </button>
                                <button className='w-full bg-slate-900 rounded-2xl px-[24px] py-[14px] flex gap-[12px] items-center justify-center shadow-sm' style={{ boxShadow: '0px 1px 2px -1px rgba(0, 0, 0, 0.1), 0px 1px 3px 0px rgba(0, 0, 0, 0.1)' }}>
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path fillRule="evenodd" clipRule="evenodd" d="M22 12C22 17.523 17.523 22 12 22C6.477 22 2 17.523 2 12C2 6.477 6.477 2 12 2C17.523 2 22 6.477 22 12ZM16.03 8.97C16.1705 9.11063 16.2493 9.30125 16.2493 9.5C16.2493 9.69875 16.1705 9.88937 16.03 10.03L11.03 15.03C10.8894 15.1705 10.6988 15.2493 10.5 15.2493C10.3012 15.2493 10.1106 15.1705 9.97 15.03L7.97 13.03C7.89631 12.9613 7.83721 12.8785 7.79622 12.7865C7.75523 12.6945 7.73319 12.5952 7.73141 12.4945C7.72963 12.3938 7.74816 12.2938 7.78588 12.2004C7.8236 12.107 7.87974 12.0222 7.95096 11.951C8.02218 11.8797 8.10701 11.8236 8.2004 11.7859C8.29379 11.7482 8.39382 11.7296 8.49452 11.7314C8.59522 11.7332 8.69454 11.7552 8.78654 11.7962C8.87854 11.8372 8.96134 11.8963 9.03 11.97L10.5 13.44L12.735 11.205L14.97 8.97C15.1106 8.82955 15.3012 8.75066 15.5 8.75066C15.6988 8.75066 15.8894 8.82955 16.03 8.97Z" fill="white" />
                                    </svg>

                                    <p className='text-lg font-semibold text-white font-bricolagegrotesque leading-[28px]'>{reservationButtonText}</p>
                                </button>
                            </div>
                        </div>
                        ...
                        <div className={`fixed bottom-0 left-0 sm:left-0 ${isSidebarCollapsed ? 'md:left-[calc(2%+80px+2px)] lg:left-[calc(2%+80px+2px)] xl:left-[calc(2%+80px+2px)]' : 'md:left-[calc(2%+22%+1px)] lg:left-[calc(2%+22%+1px)] xl:left-[calc(2%+22%+1px)]'} right-0 flex flex-row gap-[12px] items-center justify-center sm:justify-center md:justify-start lg:justify-start xl:justify-start w-full px-4 sm:px-4 md:px-5 lg:px-5 xl:px-5 py-3 sm:py-3 md:py-4 lg:py-4 xl:py-4 bg-white z-20 transition-all duration-300 ease-in-out sm:hidden md:flex`}>
                            <div className='flex flex-row gap-[12px] md:ml-2 lg:ml-[80px] xl:ml-[80px]'>
                                <button className='h-[40px] sm:h-[38px] md:h-[38px] lg:h-[38px] xl:h-[38px] rounded-[12px] px-[16px] flex items-center justify-center whitespace-nowrap min-w-[150px] sm:min-w-[150px] md:min-w-[150px] lg:min-w-[171px] xl:min-w-[171px]' style={{ background: 'radial-gradient(262.5% 262.5% at 50% -97.5%, var(--colors-teal-400, #2DD4BF) 0%, var(--colors-sky-500, #0EA5E9) 55%, var(--colors-fuchsia-500, #D946EF) 100%)' }}>
                                    <p className='text-sm text-white font-normal text-center font-bricolagegrotesque whitespace-nowrap'>{contactButtonText}</p>
                                </button>
                                <button className='w-full sm:w-[150px] md:w-[150px] lg:w-[171px] xl:w-[171px] h-[40px] sm:h-[38px] md:h-[38px] lg:h-[38px] xl:h-[38px] bg-slate-950 rounded-[12px] px-[16px] flex items-center justify-center'>
                                    <p className='text-sm text-white font-normal text-center font-bricolagegrotesque'>{reservationButtonText}</p>
                                </button>
                            </div>
                        </div>
                    </div>
