...
const ItemCard: React.FC<ItemCardProps> = ({ id, type, hotel, service, experience, health, onClick, onImageClick }) => {
  ...
  // Touch/swipe state for mobile sliding
  ...
  // Measure container width for accurate sliding (mobile only)
  useEffect(() => {
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    // Apply boundary constraints - prevent dragging beyond first/last image
    const container = e.currentTarget as HTMLElement;
    const containerWidth = container?.clientWidth || 0;
    const maxOffset = containerWidth * 0.3; // Allow 30% drag resistance
    
    let constrainedOffset = offset;
    
    // If at first image and dragging right, limit the offset
    if (currentIndex === 0 && offset > 0) {
      constrainedOffset = Math.min(offset, maxOffset);
    }
    // If at last image and dragging left, limit the offset
    else if (currentIndex === images.length - 1 && offset < 0) {
      constrainedOffset = Math.max(offset, -maxOffset);
    }
    
    setTouchOffset(constrainedOffset);
  };
  ...
  return (
    ...
    >
      ...
      >
        {/* Images container - fade on desktop, slide on mobile */}
        {isDesktop ? (
          ...
          <div
            ...
            style={{
              transform: containerWidth > 0
                ? `translateX(calc(-${currentIndex * containerWidth}px + ${touchOffset}px))`
                : `translateX(calc(-${currentIndex * (100 / images.length)}% + ${touchOffset}px))`,
        ...
        <div className="absolute top-[9px] left-[9px] right-[9px] flex justify-between items-center z-10">
          {/* Heart button */}
          <button 
            className="w-[34px] h-[34px] flex items-center justify-center transition-all group/heart z-20"
            onClick={(e) => {
              e.stopPropagation();
              handleFavorite();
            }}
          ...
          {type === 'Hotel' && (
            <button 
              ...
              className="w-[46px] h-[46px] flex items-center justify-center transition-all group/comparator z-20"
            >
          ...
          {type === 'Health' && (
            <button 
              onClick={(e) => e.stopPropagation()}
              className="w-[46px] h-[46px] flex items-center justify-center transition-all group/bookmark z-20"
            >
        ...
        <button
          ...
          className="absolute top-1/2 left-[9px] -translate-y-1/2 w-[34px] h-[34px] flex items-center justify-center bg-black/40 backdrop-blur-sm text-white rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-black/60 z-20"
        >
          <ChevronLeft size={20} />
        </button>
        <button
          ...
          className="absolute top-1/2 right-[9px] -translate-y-1/2 w-[34px] h-[34px] flex items-center justify-center bg-black/40 backdrop-blur-sm text-white rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-black/60 z-20"
        >
          <ChevronRight size={20} />
        </button>

        {/* Carousel indicators */}
        <div className="absolute bottom-[20px] left-1/2 -translate-x-1/2 flex items-center gap-[8px] z-20">
          {images.map((_, i) => (
            <button
              key={i}
              onClick={(e) => {
      ...
      <button onClick={onClick} className="w-full">
        <div className="flex flex-col gap-[14px] sm:gap-[15px] md:gap-[14px] lg:gap-[14px] xl:gap-[14px] items-start w-full">
          ...
          <div className="flex flex-col gap-[2px] sm:gap-[10px] md:gap-[2px] lg:gap-[2px] xl:gap-[2px] items-start w-full">
            ...
            <p className="text-[14px] sm:text-[16px] md:text-[16px] lg:text-[14px] xl:text-[16px] font-normal text-slate-600 leading-[20px] font-vendsans font-weight-400 text-start w-full truncate">
          ...
          {/* Price section */}
          <div className="flex items-stretch h-[36px] sm:h-[45px] md:h-[36px] lg:h-[36px] xl:h-[36px] w-full">
            {/* Total price */}
            {type === 'Health' ? (
              <div className={`flex items-center justify-center px-[10px] py-[8px] rounded-[12px] w-full ${health?.status === 'Ouvert' ? 'bg-green-100' : 'bg-red-100'}`}>
                <p className={`text-[16px] sm:text-[20px] md:text-[16px] lg:text-[14px] xl:text-[16px] font-bold ${health?.status === 'Ouvert' ? 'text-green-600' : 'text-red-600'} leading-[16px] font-bricolagegrotesque`}>
                  {health?.status}
                </p>
              </div>
            ) : (
              <>
                <div
                  className="flex items-center justify-center px-[12px] rounded-tl-[12px] rounded-bl-[12px] sm:rounded-tl-[14px] sm:rounded-bl-[14px] md:rounded-tl-[12px] md:rounded-bl-[12px] lg:rounded-tl-[10px] lg:rounded-bl-[10px] xl:rounded-tl-[12px] xl:rounded-bl-[12px]"
                  style={{
                ...
                {/* Additional info */}
                <div className="bg-green-100 flex items-center px-[10px] rounded-tr-[12px] rounded-br-[12px] sm:rounded-tr-[14px] sm:rounded-br-[14px] md:rounded-tr-[12px] md:rounded-br-[12px] lg:rounded-tr-[10px] lg:rounded-br-[10px] xl:rounded-tr-[12px] xl:rounded-br-[12px]"
                  style={{
          ...
          {/* Rating and distance */}
          <div className="flex justify-start gap-[12px] sm:gap-[9px] md:gap-[12px] lg:gap-[12px] xl:gap-[12px] items-center w-full h-[24px] sm:h-[36px] md:h-[24px] lg:h-[24px] xl:h-[24px]">
            {/* Rating */}
            <div className="flex gap-[6px] sm:gap-[9px] md:gap-[9px] lg:gap-[9px] xl:gap-[9px] items-center justify-center">
              <p className="text-[16px] sm:text-[16px] md:text-[16px] lg:text-[14px] xl:text-[16px] font-semibold text-slate-800 font-bricolagegrotesque">
                {rating}
              </p>
            ...
            {/* Number of ratings */}
            <p className="text-[14px] sm:text-[16px] md:text-[16px] lg:text-[14px] xl:text-[16px] font-semibold text-slate-600 leading-[20px] font-bricolagegrotesque">
            ...
            {/* Distance */}
            <p className="text-[14px] sm:text-[16px] md:text-[16px] lg:text-[14px] xl:text-[16px] font-normal text-slate-600 leading-[20px] font-bricolagegrotesque">
          ...
          {/* Health contact info */}
          {type === 'Health' && (
            <div className="flex gap-[6px] items-start w-full h-[36px] sm:h-[45px] md:h-[36px] lg:h-[36px] xl:h-[36px] mb-2">
              {/* Contactez Button */}
              <button
                className="flex-1 rounded-[13px] sm:rounded-[12px] md:rounded-[13px] lg:rounded-[13px] xl:rounded-[13px] h-full px-[15px] py-[6px] flex items-center justify-center gap-[4px] cursor-pointer hover:opacity-90 transition-opacity"
                style={{
              ...
              >
                ...
                <p className="text-[16px] sm:text-[18px] md:text-[16px] lg:text-[16px] xl:text-[16px] font-bricolagegrotesque font-medium leading-[24px] text-white whitespace-nowrap">
                  Contactez
                </p>
