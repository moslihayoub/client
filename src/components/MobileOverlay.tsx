...
function MobileOverlay({ setIsMobileSearchOpen }: { setIsMobileSearchOpen: (isOpen: boolean) => void }) {
  ...
  return (
    <div
      className={`fixed inset-0 bg-black/50 backdrop-blur-sm z-30 flex flex-col justify-end items-center pb-8 transition-opacity duration-300 ${
        isVisible ? 'opacity-100' : 'opacity-0'
      }`}
    >
      <div
        className={`flex flex-col gap-2 items-end justify-end h-auto w-full max-w-md px-4 transition-transform duration-300 ease-in-out ${
